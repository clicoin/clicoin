...
###   This script attempts to download the signature file SHA256SUMS.asc from clicoin.org
...
WORKINGDIR="/tmp/clicoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://clicoin.org/bin/"
VERSIONPREFIX="clicoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'clicoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Clicoin Core binary release signing key installed?"
   fi
