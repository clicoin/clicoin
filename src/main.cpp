...
#if defined(NDEBUG)
# error "Clicoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Clicoin Signed Message:\n";
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s CIC additional fees, %d delta bytes\n",
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock invocations which don't have a hash.////////////////
                         !((pindex->nHeight==0 && pindex->GetBlockHash() == uint256S("0x000000fb5bdc8e654374980951a0d9e26eff78e301d878c7b6ad7737d9628c54")) ||
                           (pindex->nHeight==0 && pindex->GetBlockHash() == uint256S("0x000000fb5bdc8e654374980951a0d9e26eff78e301d878c7b6ad7737d9628c54")));
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    if ((!CheckBlock(block, state)) || !ContextualCheckBlock(block, state, pindex->pprev)) {
        ...
        // Get prev block index  /////////////////2017-4-28,clicoin cheng add time 10 * 60 every block
        CBlockIndex* pindexPrev = NULL;
        BlockMap::iterator mi = mapBlockIndex.find(block.hashPrevBlock);
        pindexPrev = (*mi).second;

        
     ////////// Check timestamp against prev/////////2017-4-28,clicoin cheng add time 10 * 60 every block ///////////
    ////////////if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast())
    if (block.GetBlockTime() <= pindexPrev->nTime + 10 * 60)
 ////////////return state.Invalid(error("%s: block's timestamp is too early", __func__),
        return state.Invalid(error("%s: block's timestamp is too early", __func__),
                             REJECT_INVALID, "time-too-old");
